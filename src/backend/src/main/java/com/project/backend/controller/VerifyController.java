...
public class VerifyController {
    ...
    public ResponseEntity<String> verify(@RequestBody VerifyRequest request,
        ...
        if (!location.getQrToken().equals(request.qrToken())) {
            return ResponseEntity.status(403).body("Invalid QR token");
        }
        ...
        return ResponseEntity.ok("Access granted for " + user.getUsername());
    }
}
